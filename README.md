...
> [!IMPORTANT]
> This is a narrowly scoped Backstage fork of
> [`changesets/action`](https://github.com/changesets/action), used only by
> [`backstage/community-plugins`](https://github.com/backstage/community-plugins).
> Its sole intended behavior change is the `versionBranch` input, which lets
> each independently released community-plugins workspace maintain its own
> stable version pull request branch. Upstream does not currently provide a way
> to customize that head branch. All other behavior should stay aligned with
> the upstream v1 action; `backstage/backstage` uses the official action
> directly.

## Releasing this fork

Releases are intentionally manual. Before releasing, update the version in
`package.json` and add the corresponding entry to `CHANGELOG.md`. From a clean,
...
The release script creates and pushes the full version tag (for example,
`v3.0.0`) and force-updates its major release branch (for example, `v3`) to the
built release commit. Once it completes, create a GitHub release from the full
version tag and generate the release notes. Finally, update the immutable
action SHA used by `backstage/community-plugins` to the commit referenced by
the new tag.
...
- versionBranch - Sets the head branch used for the version pull request. Defaults to `changeset-release/BASE_BRANCH`
